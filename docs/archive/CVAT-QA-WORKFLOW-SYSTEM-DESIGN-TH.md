...
> เอกสารประวัติการทดลอง 16–17 กันยายน 2026 ไม่ใช่คู่มือ implementation ปัจจุบัน ดู [คู่มือหลัก](../03_CVAT-MINIO-BACKEND-END-TO-END-TH.md)
...
สถานะ workflow ที่ยืนยันจากการทดลองจริง: Organization `ptt-demo`, Project `ptt2` (#3), Task `train` (#2), Job #2 จำนวน 20 ภาพ; `annotator01` ทำ annotation, `reviewer01` ตรวจพบ Issue 1 จุด, มีการส่งกลับแก้และ Resolve แล้ว และ Job จบที่ `Acceptance / Completed` พร้อม export 241 annotations จาก 26 classes

ข้อสรุปที่ปรับใช้กับบริบทปัจจุบัน: ไม่ใช้ ZIP/export เป็นฐานข้อมูลหรือช่องทาง sync หลัก ไฟล์ export เป็น snapshot ที่สร้างเมื่อ release เท่านั้น ส่วนสถานะและ workflow อ่านจาก CVAT API/Webhook
